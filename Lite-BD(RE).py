...
# ============================================================================
# STAGE 1: Random Resize & Pad with Super-Resolution Recovery
# ============================================================================

from basicsr.archs.rrdbnet_arch import RRDBNet
from realesrgan import RealESRGANer

def load_esr_model(device='cuda'):
    """Load Real-ESRGAN model for super-resolution"""
    model = RRDBNet(num_in_ch=3, num_out_ch=3, num_feat=64, num_block=23, num_grow_ch=32, scale=4)
    upsampler = RealESRGANer(
        scale=4,
        model_path='SR_models/RealESRGAN_x4plus.pth',
        model=model,
        tile=0,
        tile_pad=10,
        pre_pad=0,
...
def sr_upsample_to_size(img_cropped, target_H, target_W, upsampler, device='cuda'):
    """
    Uses Real-ESRGAN to upsample cropped image to target size.
    
    Args:
        img_cropped: Tensor (C, H, W) in [0, 1] range (unnormalized)
        target_H, target_W: Target dimensions
        upsampler: Real-ESRGAN model
        device: torch device
    ...
    # Convert to PIL → NumPy (HWC BGR format for Real-ESRGAN)
    pil_img = transforms.ToPILImage()(img_cropped.cpu())
    img_np = np.array(pil_img)[..., ::-1]  # RGB → BGR
    
    try:
        # Apply Real-ESRGAN (4x upscale by default)
        sr_img, _ = upsampler.enhance(img_np, outscale=4.0)
        
        # Resize to exact target size
        sr_resized = cv2.resize(sr_img, (target_W, target_H), interpolation=cv2.INTER_AREA)
        
    except Exception as e:
        print(f"SR failed: {e}, falling back to bilinear interpolation")
        # Fallback to bilinear if SR fails
        sr_resized = cv2.resize(img_np, (target_W, target_H), interpolation=cv2.INTER_LINEAR)
    
    # Convert back to tensor (BGR → RGB → Tensor)
    sr_tensor = transforms.ToTensor()(
        Image.fromarray(cv2.cvtColor(sr_resized, cv2.COLOR_BGR2RGB))
    ).to(device)
    
    return sr_tensor

def random_resize_pad_with_sr(batch_tensor, upsampler, min_scale=0.75, return_defended=False):
    """
    Applies Random Resizing and Padding defense with Super-Resolution recovery.
    
    Args:
        batch_tensor: Tensor (B, C, H, W) with values in [0, 1] range (unnormalized).
        upsampler: Real-ESRGAN model for super-resolution.
    ...
    for img in batch_tensor:
        ...
        # --- Step 2: Recovery with Super-Resolution ---
        
        # Crop out the padding
        img_cropped = img_defended[:, pad_top:pad_top+new_H, pad_left:pad_left+new_W]
        
        # Super-resolution upsample to original size
        img_recovered = sr_upsample_to_size(img_cropped, H, W, upsampler, device)
...
def two_stage_defense_pipeline(img_tensor, net, target_label, clean_label,
                               mean, std, upsampler, device='cuda',
                               # Stage 1 params
                               min_scale=0.75,
                               # Stage 2 params
                               num_bands=20,
    ...
    1. Stage 1: Random Resize & Pad with SR Recovery
    ...
    if verbose:
        print("\n" + "="*80)
        print("TWO-STAGE DEFENSE PIPELINE")
        print("="*80)
    ...
    # ========================================================================
    # STAGE 1: Random Resize & Pad with SR Recovery
    # ========================================================================
...
def evaluate_two_stage_defense(args, net, poisoned_testloader, clean_labels,
                               target_label, upsampler, device='cuda', N=100,
                               # Stage 1 params
                               min_scale=0.75,
                               # Stage 2 params
                               num_bands=20,
    ...
    """
    Evaluate the two-stage defense pipeline
    """
    ...
    progress_bar = tqdm(
        total=N,
        desc=f"Two-Stage Defense [{args.dataset}][{args.atk}]",
        unit="img"
    )
    ...
    # Print statistics
    print(f"\n{'='*80}")
    print(f"Two-Stage Defense Results")
    print(f"{'='*80}")
    ...
    print(f"  Stage 1 (Resize+Pad+SR): {stage1_count}/{total} ({stage1_count/total*100:.1f}%)")
...
def visualize_defense_samples(args, net, poisoned_testloader, clean_labels,
                              target_label, upsampler, device='cuda',
                              num_samples=5,
                              # Stage 1 params
                              min_scale=0.75,
                              # Stage 2 params
                              ...
                              recovery_strength=0.3):
    """
    Visualize samples before and after two-stage defense
    ...
    plt.suptitle(
        f'Two-Stage Defense Visualization\n'
...
parser = argparse.ArgumentParser(description='Two-Stage Defense Evaluation.')
...
# Stage 1 params
parser.add_argument('--min_scale', type=float, default=0.50, help='Minimum scale factor for stage 1')
...
class PoisonedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #if self.args.atk == 'lira': poisoned_tensor = self.trigger_function(self.args, img_tensor)
...
for dataset_name in datasets:
    ...
    # Load Real-ESRGAN model once per dataset
    print(f"\nLoading Real-ESRGAN model for {args.dataset}...")
    upsampler = load_esr_model(device=device)
    ...
    for atk in attacks:
        ...
        visualize_defense_samples(
                ...
                upsampler=upsampler,
                device=device,
                num_samples=5,  # Adjust number of samples to visualize
        ...
        #print(f'PA: {clean_acc}, ASR: {filtered_asr}')
...
print("\n" + "="*100)
print("FINAL RESULTS SUMMARY - TWO-STAGE DEFENSE PIPELINE")
print("="*100)
print(results_df.to_string(index=False))
print("\n" + "="*100)

# Save results to CSV
results_df.to_csv(f'litebd_re_results.csv', index=False)
print(f"\nResults saved to: litebd_re_results.csv")